...
const Navigation = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-colors duration-300 ${scrolled ? 'bg-black shadow-md' : 'bg-black/90'}`}>
      <div className="container flex justify-between items-center h-20 px-4 md:px-8">
        {/* Logo */}
        <div className="h-10">
          <Link to="/" className="inline-block">
            <svg viewBox="0 0 63 20" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMin meet" width="100%" height="100%" className="text-white">
        ...
        {/* Mobile Menu */}
        {mobileMenuOpen && (
          <div className="md:hidden absolute top-20 left-0 w-full bg-black py-4 shadow-lg z-50">
            <ul className="flex flex-col items-center gap-4">
              <li>
                <Link to="/premium" className="text-white font-bold text-base py-2 block" onClick={() => setMobileMenuOpen(false)}>
                  Premium
                </Link>
              </li>
              <li>
                <Link to="/support" className="text-white font-bold text-base py-2 block" onClick={() => setMobileMenuOpen(false)}>
                  Support
                </Link>
              </li>
              <li>
                <Link to="/download" className="text-white font-bold text-base py-2 block" onClick={() => setMobileMenuOpen(false)}>
                  Download
                </Link>
              </li>
              <li>
                <Link to="/signup" className="text-white font-bold text-base py-2 block" onClick={() => setMobileMenuOpen(false)}>
                  Sign up
                </Link>
              </li>
              <li>
                <Link to="/login" className="text-white font-bold text-base py-2 block" onClick={() => setMobileMenuOpen(false)}>
                  Log in
                </Link>
              </li>
            </ul>
